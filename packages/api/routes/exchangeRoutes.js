 ...
 * *original* invoice. A second entry here would double-count it. Every other
 * tender on the replacement invoice (cash, card, GCash, cheque, wallet,
 * on-account) posts to ar_ledger exactly as invoiceRoutes.js already does.
 */
...
const { calculateInvoiceTax, storeTaxBreakdown, validateTaxCalculation, computeTaxForBase } = require('../services/taxCalculationService');
...
// in one atomic transaction, settling only the net difference.
router.post('/invoices/exchange', protect, hasPermission('invoicing:create'), async (req, res) => {
    const {
        ...
        payment_terms_days,
        payments,
        // Required only when the exchange owes the customer money (a downgrade)
        // and the sale is not on credit terms — see PRD Decision #2. Ignored
        // for credit-term customers, where a downgrade always reduces A/R.
        downgrade_disposition, // 'wallet' | 'cash_payout'
        physical_receipt_no,
        notes,
    } = req.body;

    if (!original_invoice_id || !employee_id) {
        return res.status(400).json({ message: 'original_invoice_id and employee_id are required.' });
    }
    if (!Array.isArray(returned_lines) || returned_lines.length === 0) {
        return res.status(400).json({ message: 'At least one returned line is required.' });
    }
    if (!Array.isArray(replacement_lines) || replacement_lines.length === 0) {
        return res.status(400).json({ message: 'At least one replacement line is required.' });
    }
    ...
    try {
        ...
        if (['Cancelled', 'Written Off'].includes(originalInvoice.status)) {
            await client.query('ROLLBACK');
            return res.status(400).json({ message: `Cannot exchange items from a ${originalInvoice.status} invoice.` });
        }
        ...
        for (const line of returned_lines) {
            ...
            if (!invoice_line_id || !(Number(quantity) > 0)) {
                await client.query('ROLLBACK');
                return res.status(400).json({ message: 'Each returned line needs invoice_line_id and a positive quantity.' });
            }
            ...
            if (Number(quantity) > availableToReturn) {
                ...
                return res.status(400).json({
                    message: `Exchange failed for part_id ${lineData.part_id}: requested ${quantity}, available ${availableToReturn}.`
                });
            }
        ...
        if (parts.length !== new Set(partIds).size) {
            await client.query('ROLLBACK');
            return res.status(400).json({ message: 'One or more replacement part_id values were not found.' });
        }
        ...
        if (!termsValidation.isValid) {
            await client.query('ROLLBACK');
            return res.status(400).json({ message: 'Invalid payment terms', errors: termsValidation.errors });
        }
        ...
        if (isCreditSale && customerRow.credit_hold) {
            ...
            if (!hasOverrideParam && !hasManagerPermission) {
                ...
                return res.status(403).json({
                    message: `Exchange blocked: Customer is on credit hold (${customerRow.credit_hold_reason || 'Credit Hold'}). Manager override required.`,
                    credit_hold: true, reason: customerRow.credit_hold_reason,
                });
            }
        }
        ...
        if (leftoverCredit > 0 && !isCreditSale && !['wallet', 'cash_payout'].includes(downgrade_disposition)) {
            await client.query('ROLLBACK');
            return res.status(400).json({
                message: `This exchange owes the customer ₱${leftoverCredit.toFixed(2)}. Specify downgrade_disposition: 'wallet' or 'cash_payout'.`
            });
        }
        if (amountDue > 0 && !isCreditSale) {
            ...
            if (totalTendered !== amountDue) {
                await client.query('ROLLBACK');
                return res.status(400).json({
                    message: `This exchange requires ₱${amountDue.toFixed(2)} to be tendered now; received ₱${totalTendered.toFixed(2)}.`
                });
            }
        }
        ...
        if (Array.isArray(payments) && payments.length > 0) {
            ...
            if (totalPayments > amountDue + 0.01) {
                await client.query('ROLLBACK');
                return res.status(400).json({ message: `Tendered ₱${totalPayments.toFixed(2)} exceeds the ₱${amountDue.toFixed(2)} due on this exchange.` });
            }
            ...
            for (const payment of payments) {
                ...
                if (methodConfig.requires_reference && (!reference || reference.trim() === '')) {
                    await client.query('ROLLBACK');
                    return res.status(400).json({ message: `Reference is required for ${method.name}` });
                }
                if (methodConfig.requires_receipt_no && !prn) {
                    await client.query('ROLLBACK');
                    return res.status(400).json({ message: `Physical receipt number is required for ${method.name}` });
                }
                ...
                if (changeAmt > 0 && !methodConfig.change_allowed) {
                    await client.query('ROLLBACK');
                    return res.status(400).json({ message: `Change is not allowed for ${method.name}` });
                }
