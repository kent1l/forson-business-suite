                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                                {sortedReportData.map((row) => (
                                    <tr key={row.cn_id} className="hover:bg-gray-50 dark:hover:bg-slate-700/40 text-gray-800 dark:text-slate-200 transition-colors">
                                        <td className="p-3 text-sm font-mono text-gray-900 dark:text-slate-100">{row.cn_number}</td>
                                        ...
                                        <td className="p-3 text-sm font-medium">{row.customer_name}</td>
                                        <td className="p-3 text-sm text-right font-mono font-semibold text-danger-600 dark:text-danger-400">{settings?.DEFAULT_CURRENCY_SYMBOL || '₱'}{parseFloat(row.total_amount).toFixed(2)}</td>
                                    </tr>
                                ))}
                                 {reportData.length === 0 && !loading && (
                                    <tr>
                                        <td colSpan="5" className="p-4 text-center text-gray-500 dark:text-slate-400">No refunds found for the selected period.</td>
                                    </tr>
                                )}
                            </tbody>
                    </table>
