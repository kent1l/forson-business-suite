                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                                {sortedReportData.map((row) => (
                                    <tr key={row.customer_id} className="hover:bg-gray-50 dark:hover:bg-slate-700/40 text-gray-800 dark:text-slate-200 transition-colors">
                                        <td className="p-3 text-sm font-medium text-gray-900 dark:text-slate-100">{row.first_name} {row.last_name}</td>
                                        <td className="p-3 text-sm text-center">{row.total_invoices}</td>
                                        <td className="p-3 text-sm text-right font-mono font-semibold text-gray-900 dark:text-slate-100">{settings?.DEFAULT_CURRENCY_SYMBOL || '₱'}{parseFloat(row.total_sales).toFixed(2)}</td>
                                    </tr>
                                ))}
                                {sortedReportData.length === 0 && (
                                    <tr>
                                        <td colSpan="3" className="p-4 text-center text-gray-500 dark:text-slate-400">No data to display.</td>
                                    </tr>
                                )}
                            </tbody>
                    </table>
