...
const ErrorState = ({ title = 'Something went wrong', description = 'Failed to load this data. Please try again.', onRetry, className = '' }) => (
    <div className={`flex flex-col items-center justify-center gap-2 py-12 text-center ${className}`}>
        <div className="flex h-12 w-12 items-center justify-center rounded-full bg-danger-50 dark:bg-danger-900/30">
            <Icon path={ICONS.warning} className="h-6 w-6 text-danger-600" />
        </div>
        <p className="text-sm font-medium text-danger-700 dark:text-danger-500">{title}</p>
        {description && <p className="max-w-sm text-xs text-neutral-400 dark:text-slate-500">{description}</p>}
        {onRetry && (
            <button
                type="button"
                onClick={onRetry}
                className="mt-2 rounded-md border border-neutral-300 dark:border-slate-600 px-3 py-1.5 text-xs font-semibold text-neutral-700 dark:text-slate-300 transition-colors hover:bg-neutral-50 dark:hover:bg-slate-800"
            >
                Retry
            </button>
        )}
