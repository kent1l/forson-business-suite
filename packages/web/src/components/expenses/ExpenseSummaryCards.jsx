        ...
        <div className="space-y-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between">
                    <div>
                        <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Filtered Total Expenses</p>
                        <h3 className="text-2xl font-bold text-slate-900 dark:text-slate-100 mt-1">{formatCurrency(totalCurrentMonth)}</h3>
                        <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">{categorySummary.reduce((a, b) => a + (b.count || 0), 0)} logged expenses</p>
                    </div>
                    <div className="p-3 bg-primary-50 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400 rounded-xl border border-primary-100 dark:border-primary-800">
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between">
                    <div>
                        <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Top Expense Category</p>
                        <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mt-1 truncate max-w-[200px]">
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between">
                    <div>
                        ...
                        <h3 className="text-2xl font-bold text-slate-900 dark:text-slate-100 mt-1">{categorySummary.length}</h3>
                        <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">With logged expenses in period</p>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                        <div>
                            <h4 className="text-sm font-bold text-slate-800 dark:text-slate-100 uppercase tracking-wider">Monthly Expense Trend</h4>
                            <p className="text-xs text-slate-400 dark:text-slate-400">Month-over-month total operating expenses (Last 12 Months)</p>
                        </div>
                    </div>
                        ...
                        {monthlySummary.length === 0 ? (
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={monthlySummary} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                                    ...
                                    />
                                    <Bar dataKey="total_amount" fill="#0284c7" radius={[4, 4, 0, 0]} />
                                </BarChart>
                            </ResponsiveContainer>
                        )}
                    </div>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm">
                    <h4 className="text-sm font-bold text-slate-800 dark:text-slate-100 uppercase tracking-wider mb-1">Expenses by Category</h4>
                    <p className="text-xs text-slate-400 dark:text-slate-400 mb-4">Breakdown of operating costs</p>

                    <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
                        {categorySummary.length === 0 ? (
                            <p className="text-xs text-slate-400 dark:text-slate-500 text-center py-6">No category data in date range</p>
                        ) : (
                                ...
                                return (
                                    <div key={cat.category_id || idx} className="space-y-1">
                                        <div className="flex items-center justify-between text-xs font-medium">
                                            <span className="text-slate-700 dark:text-slate-300 font-semibold truncate max-w-[140px]">{cat.category_name}</span>
                                            <span className="text-slate-900 dark:text-slate-100 font-bold">{formatCurrency(cat.total_amount)}</span>
                                        </div>
