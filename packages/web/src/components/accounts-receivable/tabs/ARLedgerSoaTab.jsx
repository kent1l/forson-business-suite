...
const ARLedgerSoaTab = ({
    ...
    return (
        <div className="space-y-6">
            <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-gray-200 dark:border-slate-700 shadow-card flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div className="w-full md:w-96 relative" ref={soaComboboxRef}>
                    <label className="block text-xs font-semibold uppercase text-gray-600 dark:text-slate-400 mb-1">Search Customer</label>
                    <div className="relative">
                        <input
                            ...
                            className="w-full px-3 py-2 pr-8 border border-gray-300 dark:border-slate-600 rounded-lg text-sm bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 placeholder:text-gray-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-primary-500"
                        />
                        {soaSearchQuery && (
                            <button
                                type="button"
                                ...
                                className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-slate-300 focus:outline-none p-1 rounded-full hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors"
                                title="Clear search"
                            >
                    ...
                    {soaDropdownOpen && (
                        <div className="absolute z-50 left-0 right-0 mt-1 max-h-60 overflow-y-auto bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-lg">
                            {filteredSoaCustomers.length === 0 ? (
                                <div className="p-3 text-xs text-gray-500 dark:text-slate-400 text-center">No matching customer accounts found</div>
                            ) : (
                                filteredSoaCustomers.map((c, idx) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`w-full text-left px-3 py-2 text-sm flex justify-between items-center transition-colors border-b border-gray-100 dark:border-slate-700/60 last:border-0 ${
                                                isHighlighted
                                                    ? 'bg-primary-100 dark:bg-primary-900/40 font-semibold text-primary-900 dark:text-primary-200 ring-1 ring-primary-300 dark:ring-primary-700'
                                                    : isSelected
                                                    ? 'bg-primary-50 dark:bg-primary-950/40 font-semibold text-primary-700 dark:text-primary-300'
                                                    : 'text-gray-700 dark:text-slate-200 hover:bg-gray-50 dark:hover:bg-slate-700/50'
                                            }`}
                                        >
                                            <span className="truncate">{displayName}</span>
                                            <span className="font-mono text-xs text-gray-500 dark:text-slate-400 ml-2 whitespace-nowrap">
                    ...
                    <div className="flex items-center gap-4">
                        <label className="inline-flex items-center gap-2 cursor-pointer select-none">
                            <div className="relative">
                                ...
                                />
                                <div className="w-9 h-5 bg-gray-300 dark:bg-slate-600 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-emerald-600"></div>
                            </div>
                            <span className="text-xs font-semibold text-gray-700 dark:text-slate-300">Attach images</span>
                        </label>
                        <button
            ...
            {soaLoading ? (
                ...
                <div className="space-y-6">
                    <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden shadow-card">
                        <div className="p-6 border-b border-gray-100 dark:border-slate-700 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gray-50 dark:bg-slate-800/80">
                            <div>
                                <div className="flex items-center gap-2">
                                    <h3 className="text-xl font-bold text-gray-800 dark:text-slate-100">{soaLedger.customer.name}</h3>
                                    <span className="px-2.5 py-0.5 rounded bg-primary-100 dark:bg-primary-900/30 text-primary-800 dark:text-primary-300 text-xs font-mono font-semibold">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm text-left text-gray-500 dark:text-slate-400">
                                <thead className="text-xs text-gray-700 dark:text-slate-300 uppercase bg-gray-100 dark:bg-slate-700/50 border-b border-gray-200 dark:border-slate-700">
                                    <tr>
                                ...
                                <tbody className="divide-y divide-gray-200 dark:divide-slate-700/60">
                                    <tr className="bg-primary-50/60 dark:bg-primary-950/30 font-semibold text-gray-800 dark:text-slate-100">
                                        <td className="px-5 py-3 whitespace-nowrap">{dateRange.startDate.toLocaleDateString()}</td>
                                        <td className="px-5 py-3">—</td>
                                        <td className="px-5 py-3 font-mono text-xs text-gray-400 dark:text-slate-500">—</td>
                                        <td className="px-5 py-3 font-semibold text-primary-900 dark:text-primary-300">OPENING BALANCE BROUGHT FORWARD</td>
                                        <td className="px-5 py-3 text-right font-mono">—</td>
                                        <td className="px-5 py-3 text-right font-mono">—</td>
                                        <td className="px-5 py-3 text-right font-mono font-bold text-primary-900 dark:text-primary-300">{formatCurrency(soaLedger.opening_balance)}</td>
                                    </tr>
                                    {soaLedger.ledger_rows.map((row, idx) => {
                                        ...
                                        <tr key={row.ledger_id || idx} className="hover:bg-gray-50 dark:hover:bg-slate-700/40 text-gray-800 dark:text-slate-200 transition-colors">
                                                ...
                                                <div className="flex items-center gap-1.5">
                                                    ...
                                                    {canChangeDate && dateChangeTargetForRow && (
                                                        <button
                                                            ...
                                                            className="inline-flex items-center justify-center w-5 h-5 text-gray-400 dark:text-slate-500 hover:text-gray-700 dark:hover:text-slate-200 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-slate-400"
                                            ...
                                            <td className="px-5 py-3.5 whitespace-nowrap text-gray-600 dark:text-slate-400">{row.due_date ? new Date(row.due_date).toLocaleDateString() : '—'}</td>
                                            <td className="px-5 py-3.5 font-mono text-xs">
                                                <div className="font-bold text-gray-900 dark:text-slate-100">
                                                    {row.primary_ref || row.physical_receipt_no || '-'}
                                                </div>
                                                {row.sub_ref && (
                                                    <div className="text-[11px] font-normal text-gray-400 dark:text-slate-500 mt-0.5">
                                                        {row.sub_ref}
                                                    </div>
                                                )}
                                            </td>
                    ...
                    {soaLedger.pending_cheques && soaLedger.pending_cheques.length > 0 && (
                        <div className="bg-amber-50/70 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/60 rounded-xl p-5 shadow-sm">
                            <div className="flex justify-between items-center mb-3">
                                <h4 className="text-sm font-bold text-amber-900 dark:text-amber-300 flex items-center gap-2">
                                    <Icon path={ICONS.history} className="w-4 h-4 shrink-0 text-amber-700 dark:text-amber-400" />
                                    ...
                                    <span className="px-2 py-0.5 bg-amber-200 dark:bg-amber-900/50 text-amber-900 dark:text-amber-200 rounded-full text-xs font-semibold">
                                        {soaLedger.pending_cheque_count} Items
                                    </span>
                                </h4>
                                <div className="text-sm font-bold font-mono text-amber-950 dark:text-amber-200">
                            ...
                            <div className="overflow-x-auto bg-white dark:bg-slate-800 rounded-lg border border-amber-200 dark:border-amber-800/60">
                                <table className="w-full text-xs text-left text-gray-600 dark:text-slate-400">
                                    <thead className="bg-amber-100/60 dark:bg-amber-900/40 text-amber-950 dark:text-amber-200 uppercase font-semibold border-b border-amber-200 dark:border-amber-800/60">
                                        <tr>
                                    ...
                                    <tbody className="divide-y divide-amber-100 dark:divide-slate-700/60">
                                        {soaLedger.pending_cheques.map((item) => (
                                            <tr key={item.payment_id} className="hover:bg-amber-50/30 dark:hover:bg-slate-700/40 text-gray-800 dark:text-slate-200">
                                                <td className="px-4 py-2 whitespace-nowrap text-gray-600 dark:text-slate-400">{new Date(item.cheque_date).toLocaleDateString()}</td>
                                                <td className="px-4 py-2 font-mono font-semibold text-gray-800 dark:text-slate-100">{item.reference_number || '-'}</td>
                                                <td className="px-4 py-2 text-gray-700 dark:text-slate-300">{item.payment_method_name || 'Bank Instrument'}</td>
                                                <td className="px-4 py-2 text-center">
                                                    <span className="px-2 py-0.5 rounded bg-amber-100 dark:bg-amber-900/40 text-amber-900 dark:text-amber-300 text-xs font-medium">
                                                        {item.pdc_status}
                                                    </span>
                                                </td>
                                                <td className="px-4 py-2 text-right font-mono font-bold text-gray-900 dark:text-slate-100">{formatCurrency(item.amount)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
