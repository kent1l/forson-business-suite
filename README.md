...
### Search Sync Health Metrics
For Phase 1 Meilisearch outbox monitoring, the dashboard now includes search-sync queue health:
- `GET /api/dashboard/search-sync-health` (pending/processing/done/dead counts + oldest pending age)
- `GET /api/dashboard/enhanced-stats` includes a `searchSyncHealth` object.
