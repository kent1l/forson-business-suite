...
INITIAL_TZ=$(db_setting APP_TIMEZONE "Asia/Manila")
export TZ="$INITIAL_TZ"
...
# --- Dynamic cron loop ---
CURRENT_CRON=""
CURRENT_TZ="$INITIAL_TZ"

echo "[run.sh] Starting backup scheduler..."

while true; do
    NEW_CRON=$(db_setting BACKUP_SCHEDULE_CRON "0 2 * * *")
    NEW_TZ=$(db_setting APP_TIMEZONE "Asia/Manila")

    if [ "$NEW_CRON" != "$CURRENT_CRON" ] || [ "$NEW_TZ" != "$CURRENT_TZ" ]; then
        echo "[run.sh] Config changed. Cron: '$NEW_CRON', TZ: '$NEW_TZ'"

        # Kill existing supercronic if running
        if [ -n "$SUPERCRONIC_PID" ] && kill -0 "$SUPERCRONIC_PID" 2>/dev/null; then
            echo "[run.sh] Restarting supercronic with new configuration..."
            kill "$SUPERCRONIC_PID" 2>/dev/null || true
        ...
        CURRENT_TZ="$NEW_TZ"
        echo "[run.sh] Supercronic started (PID: $SUPERCRONIC_PID) with TZ: $CURRENT_TZ"
    fi
