...
BEGIN
    ...
    LOOP
        -- Process invoices in batches to prevent WAL bloat
        invoices_to_process := ARRAY(
            SELECT invoice_id
            FROM invoice
            WHERE tax_total IS NULL 
            AND subtotal_ex_tax IS NULL
            ORDER BY invoice_id
            LIMIT batch_size
        );

        IF array_length(invoices_to_process, 1) IS NULL THEN
            EXIT; -- Complete when no more invoices need processing
        END IF;

        FOR batch_index IN 1 .. array_length(invoices_to_process, 1)
        LOOP
            SELECT invoice_id, invoice_number, total_amount INTO invoice_rec
            FROM invoice 
            WHERE invoice_id = invoices_to_process[batch_index];
        ...
        COMMIT; -- Commit the batch to free WAL space
    END LOOP;
