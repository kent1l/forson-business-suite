...
CREATE TABLE IF NOT EXISTS public.customer_payment (
    ...
    notes text,
    method_id integer REFERENCES public.payment_methods(method_id) ON DELETE SET NULL
);
...
--
-- INVOICE PAYMENTS FOR SPLIT PAYMENT SUPPORT
--
...
-- Create validation trigger (will be created after invoice_payments table exists)
...
--
-- UNIFIED PAYMENTS VIEW FOR REPORTING
--
...
('ENABLE_SPLIT_PAYMENTS', 'false'), ('PAYMENT_METHODS_HELP_TEXT', 'Configure available payment methods and their validation rules')
...
-- Seed: payment_methods (default payment methods with configurations)
